...
const createRedisConnection = async (): Promise<RedisCacheClient | null> => {
  ...
  client.on('error', (error: unknown) => {
    console.error('[order-query-cache] Redis error', error);
  });
  ...
  try {
    ...
    console.error('[order-query-cache] Falha ao conectar no Redis', error);
    try {
...
export const getCacheJson = async <TValue>(key: string): Promise<TValue | null> => {
  try {
    ...
    console.error(`[order-query-cache] Falha ao ler chave ${key}`, error);
    return null;
  }
};
...
export const setCacheJson = async <TValue>(
  ...
  try {
    ...
    console.error(`[order-query-cache] Falha ao gravar chave ${key}`, error);
  }
};
...
export const deleteCacheKey = async (key: string): Promise<void> => {
  try {
    ...
    console.error(`[order-query-cache] Falha ao remover chave ${key}`, error);
  }
};
...
export const deleteCachePattern = async (pattern: string): Promise<number> => {
  try {
    ...
    console.error(`[order-query-cache] Falha ao invalidar padrao ${pattern}`, error);
    return 0;
  }
};

export const closeOrderQueryRedisCache = async (): Promise<void> => {
  if (!redisClient) {
    return;
  }
  try {
